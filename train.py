...
def training(model,train_loader,num_epochs,g_opt,d_opt):
    ...
    img_list: list containing generated images.
    '''
    ...
    for epoch in range(num_epochs):
        ...
        for i , (img,_) in enumerate(tqdm(train_loader)):
            ...
            if (i % 50 == 0) or ((epoch == num_epochs-1) and (i == len(train_loader)-1)):
                # print(i)
                with torch.no_grad():
        ...
        print(f"Epoch {epoch}\tDiscrimnative Loss: {running_discriminator_loss/len(train_loader)} \tGenerative loss: {running_generator_loss/len(train_loader)}")
        utilities.view_images(img_list, real_batch,epoch)
        dloss.append(running_discriminator_loss/len(train_loader))
        gloss.append(running_generator_loss/len(train_loader))
    return img_list, dloss, gloss